...
if (!this.child_init) {
	...
	if (this.context_values[0] == 0) {
			this.make_trigger(this.x-32, this.y, 32, 32);
		if (this.get_ss("shoreplace", "aliph_alone", 1) == 1) {
			this.s2 = 1;
			return;
		} else {
		}
		...
		if (this.get_ss("shoreplace", "aliph_alone", 2) == 1) {
			this.s2 = 1;
			return;
		} else {
		}
...
if (this.s2 == 1) {
	if (this.try_to_talk(0, this.trigger, true)) {
			...
			if (this.context_values[0] == 0) {
				this.dialogue("shoreplace", "aliph_alone", 0);
			} else if (this.context_values[0] == 1) {
				this.dialogue("shoreplace", "aliph_alone", 1);
			}
		this.s1 = 1;
		this.s2 = 0;
	}
...
if (this.s1 == 0) {
	...
	if (this.player_freeze_help()) {
		this.s1 = 1;
		if (this.context_values[0] == 0) {
			this.set_ss("shoreplace", "aliph_alone", 1, 1);	
			this.dialogue("shoreplace", "aliph_alone", 0);
		} else if (this.context_values[0] == 1) {
			this.set_ss("shoreplace", "aliph_alone", 2, 1);
			this.dialogue("shoreplace", "aliph_alone", 1);
		}
	}
} else if (this.s1 == 1) {
