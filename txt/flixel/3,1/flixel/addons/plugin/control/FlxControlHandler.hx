...
class FlxControlHandler
{
	...
	private function moveDown():Bool
	{
		...
		if (FlxG.keys.myPressed(_downKey))
		{
			move = true;
			isPressedDown = true;
	...
	private function moveLeft():Bool
	{
		...
		if (FlxG.keys.myPressed(_leftKey))
		{
			move = true;
			isPressedLeft = true;
	...
	private function moveRight():Bool
	{
		...
		if (FlxG.keys.myPressed(_rightKey))
		{
			move = true;
			isPressedRight = true;
	...
	private function moveAntiClockwise():Bool
	{
		...
		if (FlxG.keys.myPressed(_antiClockwiseKey))
		{
			move = true;
	...
	private function moveClockwise():Bool
	{
		...
		if (FlxG.keys.myPressed(_clockwiseKey))
		{
			move = true;
	...
	private function moveThrust():Bool
	{
		...
		if (FlxG.keys.myPressed(_thrustKey))
		{
			move = true;
	...
	private function moveReverse():Bool
	{
		...
		if (FlxG.keys.myPressed(_reverseKey))
		{
			move = true;
	...
	private function runFire():Bool
	{
		...
		// 1 = Just Down
		// 2 = Just Released
		if ((_fireKeyMode == 0 && FlxG.keys.myPressed(_fireKey)) || (_fireKeyMode == 1 && FlxG.keys.myJustPressed(_fireKey)) || (_fireKeyMode == 2 && FlxG.keys.justReleased(_fireKey)))
		{
			if (_fireRate > 0)
			{
	...
	private function runJump():Bool
	{
		...
		if ((_jumpKeyMode == KEYMODE_PRESSED && FlxG.keys.myPressed(_jumpKey)) || (_jumpKeyMode == KEYMODE_JUST_DOWN && FlxG.keys.myJustPressed(_jumpKey)) || (_jumpKeyMode == KEYMODE_RELEASED && FlxG.keys.justReleased(_jumpKey)))
		{
