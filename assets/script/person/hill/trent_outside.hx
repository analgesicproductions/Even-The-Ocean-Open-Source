...
// trent outside. show up only if you saw the other memory
if (!this.child_init) {
	this.child_init = true;
	this.visible = false;
}
if (this.s1 == 0) {
	if (this.context_values[0] ==0) { // Location t1
	...
	if (this.try_to_talk()) {
		this.dialogue("hill", "trent_outside", 0);
		this.s1 = 2;
	}
	...
	if (R.TEST_STATE.cutscene_just_finished(2)) {
		...
		this.set_scene_state("hill", "trent_outside", 1,1);
