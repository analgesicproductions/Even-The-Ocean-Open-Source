...
class FlxKeyManager<Key:Int, KeyList:FlxBaseKeyList> implements IFlxInputManager
{
	...
	public function myJustPressed(s:String):Bool {
		return checkKeyArrayState([cast FlxKey.fromString(s)], JUST_PRESSED);
	}
	public function myPressed(s:String):Bool {
		return checkKeyArrayState([cast FlxKey.fromString(s)], PRESSED);
	}
