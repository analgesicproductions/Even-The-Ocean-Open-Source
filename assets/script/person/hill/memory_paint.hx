...
if (this.s1 == 0) {
	...
	//this._trace(this.context_values[0]);
	// Only one of these shows up
	if (this.context_values[0] ==0) { // Location B1
		if (this.get_scene_state("hill", "paint_memory", 1) == 2) {
			this.visible = false;
		}
	} else { // Location B2
		if (this.get_scene_state("hill", "paint_memory", 1) == 1) {
			this.visible = false;
		}
	}
	if (this.visible) {
		if (this.s2 == 0) {
			...
			this.alpha = 0.7 + 0.3 * this.get_sin(this.t_1 * 3);
			
			if (this.try_to_talk()) {
				if (this.get_ss("hill", "paint_memory", 1) > 0) {
					this.dialogue("hill", "paint_memory", 1);
					this.s2 = 2;
				} else {
					this.dialogue("hill", "paint_memory", 2);
					this.s2 = 1;
				}
			} 
	...
	if (this.s2 == 0) {
		this.dialogue("hill", "paint_memory", 3);
		this.s2 = 1;
	} else if (this.s2 == 1) {
		...
		if (this.fade_out([trent, bay]) && this.fade_in(this) && this.fade_in(R.player)) {
			...
			if (this.context_values[0] ==0) { // Location B1
				this.set_scene_state("hill", "paint_memory", 1, 1);
			} else { // Location B2
				this.set_scene_state("hill", "paint_memory", 1, 2);
			}
		}
	}
}  else if (this.s1 == 3) {
