...
class FlxTilemap extends FlxObject
{
		...
		/**
* Changes this tilemap's dimensions.
* WARNING. If w and h are less than the current size,
* you will lose data!
* Otherwise, appends 0s as needed.
* @param w
* @param h
*/
public function change_dimensions(w:Int, h:Int,graphic:BitmapData):Void {
var csv_data:Array<Int> = getData();
var new_csv:String = "";

for (i in 0...h) {
for (j in 0...w) {
if (i < heightInTiles && j < widthInTiles) {
new_csv += Std.string(getTile(j, i));
} else {
new_csv += "0";
}
...
loadMap(new_csv, graphic, 16, 16);

}	

public function getTileID(X:Float, Y:Float):Int {

var ix:Int = Std.int(X/16);
var iy:Int = Std.int(Y/16);
var tile_type:Int = _data[iy * widthInTiles + ix];
return tile_type;
}

/**
* Check the collision flags of a tile. Assumes the tilemap is at (0,0)
* @param X The x coordinate you want to look at the tilemap at.
* @param Y etc.
* @return
*/
public function getTileCollisionFlags(X:Float, Y:Float):Int
{
var ix:Int = Std.int(X/16);
var iy:Int = Std.int(Y/16);
var tile_type:Int = _data[iy * widthInTiles + ix];
var the_tile:FlxTile = _tileObjects[tile_type];
return the_tile.allowCollisions;
}
	 ...
	 */
	public var tileScaleHack:Float = 1.00005;
	/**
	 ...
	 */
	public var _buffers:Array<FlxTilemapBuffer>;
	/**
	...
	{
		if (_tileObjects[Index] == null) return 0;
