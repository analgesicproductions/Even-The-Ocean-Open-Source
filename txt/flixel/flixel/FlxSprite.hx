...
public function make_rect_outline(w:Int=1,h:Int=1,color:Int=0xffffffff,cachekey:String):Void {
makeGraphic(w, h, 0x00000000, false, cachekey + Std.string(w) + "," + Std.string(h));
for (i in 0...h) {
for (j in 0...w) {
pixels.setPixel32(j, i, 0x00000000);
}
}
for (i in 0...h) {
pixels.setPixel32(0, i, color);
pixels.setPixel32(1, i, color);
pixels.setPixel32(w - 1, i, color);
pixels.setPixel32(w - 2, i, color);
}
for (j in 0...w) {
pixels.setPixel32(j, 0, color);
pixels.setPixel32(j, 1, color);
pixels.setPixel32(j, h - 1, color);
pixels.setPixel32(j, h - 2, color);
}
}
	...
	public function myLoadGraphic(Graphic:FlxGraphicAsset, Animated:Bool = false, Reverse:Bool = false, Width:Int = 0, Height:Int = 0, Unique:Bool = false, ?Key:String, has_spaces:Bool = false):FlxSprite {
		if (has_spaces) {
			return loadGraphic(Graphic, Animated, Width, Height, Unique,"spacedsean");
		}
		return loadGraphic(Graphic, Animated, Width, Height, Unique, Key);
	}
	/**
	...
	public function loadGraphic(Graphic:FlxGraphicAsset, Animated:Bool = false, Width:Int = 0, Height:Int = 0, Unique:Bool = false, ?Key:String):FlxSprite
	{
		...
		{
			if (has_spaces) {
				frames = FlxTileFrames.fromBitmapAddSpacesAndBorders(graph, FlxPoint.get(Width, Height), new FlxPoint(1, 1), new FlxPoint(1, 1));
			} else {
...
* Fills this sprite's graphic with a specific color.
	...
	override public function draw():Void
	{
		...
		for (camera in cameras)
		{
			...
			_point.x -= (camera.scroll.x * scrollFactor.x) + (offset.x);
			_point.y -= (camera.scroll.y * scrollFactor.y) + (offset.y);
	...
	/**
	 * Scales before rotates
	 */
	private var scale_first:Bool = true;
	private function drawComplex(camera:FlxCamera):Void
	{
		_frame.prepareMatrix(_matrix, FlxFrameAngle.ANGLE_0, checkFlipX(), checkFlipY());
		_matrix.translate(-origin.x, -origin.y);
		
		if (scale_first) _matrix.scale(scale.x, scale.y);
		if (bakedRotationAngle <= 0)
		{
			updateTrig();
