...
class FlxObject extends FlxBasic
{
		...
		private var no_collide_ceil_slopes:Bool = false;
		private var no_collide_floor_slopes:Bool = false;
/**
* For dealing with overlapping on a gradient slope. The idea is that if you are on one or more gradients, you need to be forced
* "out" of the slope by a minimum amount.
*/
public var _minslopebump:Int = 0;
public var touching_floor_slope:Bool = false;
public var did_slopes:Int = 0; // 0x01 = floor, 0x10 = ceil;
...
this.y = y;
	...
	*/
	...
	private function set_height(Height:Float):Float
	{
		...
		if (Height < 0) 
		{
			FlxG.log.add(Track.get_stacktrace(true));
